...
class zhihu():
    ...
    def login(self, username, password, version='pc'):
        if version == 'mobile':
            ...
            self.headers.update(
                {'x-zse-83': '3_1.1', 'x-xsrftoken': _xsrf, 'content-type': 'application/x-www-form-urlencoded'})
            self.session.headers.update(self.headers)
            res = self.session.post(self.login_url, data=data)
            if 'user_id' in res.json():
                print('[INFO]: 用户名 -> %s, 登录成功...' % username)
                removeImage(os.path.join(self.cur_path, 'captcha.jpg'))
                print(username, self.session)
                return username, self.session
            else:
                raise RuntimeError('用户名 -> %s, 登录失败，用户名或者密码错误...' % username)
        else:
...
if __name__ == '__main__':
    zhihu().login('16608006657', '520*ihtxgq@ZH')
